...
%          - Also shows the variance in the data
%          - option to keep result matrices
%          - use parfor for parallel jobs
% 
...
V = zeros(nObsVar,nHorizons,nDrawsUsed);
parfor jd=1:nDrawsUsed
  V(:,:,jd) = MCMCVarianceObsFcn(xd(:,jd),Params,SymMats,nObsVar,nStateVar,nShockVar,np,1,...
...
znamelength = [cellfun('length',ObsVar)];
znamelengthmax = max(znamelength);
fprintf(['%-',int2str(znamelengthmax),'s',repmat('   %10.0f',1,nHorizons),'   %10s\n'],...
  'Horizon',VHorizons(:),'Data')
for jz=1:nObsVar
    fprintf(['%-',int2str(znamelengthmax),'s',repmat('   %10.6f',1,nHorizons+1),'\n'],...
      ObsVar{jz},V(jz,:),VObs(jz))
end
